...
(function () {
  ...
  if (typeof siCssDataSp === 'undefined') {
    console.warn('siCssDataSp non definito, impossibile inizializzare il CSS Class Editor');
    return;
  }
  ...
  (function injectDynamicCSS() {
    if (typeof document === 'undefined' || typeof siCssDataSp === 'undefined') {
      return;
    }
    ...
    styleEl.textContent = siCssDataSp.classes
      .map(c => `${c}`)
      .join('\n');
  })();
  ...
  function AddCSSClassControl(settings) {
    ...
    settings.edit = function (props) {
      ...
      // Filtra le classi
      const filtered = siCssDataSp.classes.filter((cls) => 
        cls.toLowerCase().includes(searchTerm.toLowerCase())
      );

      // if ( filtered == siCssDataSp.classes.toLowerCase() ) {
      //   filtered = [];
      // }
        ...
        props.isSelected &&
          createElement(
            ...
            createElement(
              PanelBody,
              {
                title: siCssDataSp.labels.title || 'Spectre Classes',
                initialOpen: false,
                className: 'studioimmens-css-control',
              },
              // Campo di ricerca
              createElement(TextControl, {
                label: siCssDataSp.labels.search || 'Search Classes',
                value: searchTerm,
                onChange: setSearchTerm,
                placeholder: siCssDataSp.labels.ttos || 'Type to search...',
              }),
                ...
                ? createElement(
                    ...
                    filtered.length > 0
                      ...
                      : createElement(
                          'p',
                          {},
                          siCssDataSp.labels.nofound || 'No classes found'
                        )
                  )
                : null
            )
