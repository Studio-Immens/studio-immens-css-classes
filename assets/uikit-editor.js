...
(function () {
  ...
  if (typeof siCssDataUi === 'undefined') {
    console.warn('siCssDataUi non definito, impossibile inizializzare il CSS Class Editor');
    return;
  }
  ...
  (function injectDynamicCSS() {
    if (typeof document === 'undefined' || typeof siCssDataUi === 'undefined') {
      return;
    }
    ...
    styleEl.textContent = siCssDataUi.classes
      .map(c => `${c}`)
      .join('\n');
  })();
  ...
  function AddCSSClassControl(settings) {
    ...
    settings.edit = function (props) {
      ...
      // Filtra le classi
      const filtered = siCssDataUi.classes.filter((cls) => 
        cls.toLowerCase().includes(searchTerm.toLowerCase())
      );

      // if ( filtered == siCssDataUi.classes.toLowerCase() ) {
      //   filtered = [];
      // }
        ...
        props.isSelected &&
          createElement(
            ...
            createElement(
              PanelBody,
              {
                title: siCssDataUi.labels.title || 'Uikit Classes',
                initialOpen: false,
                className: 'studioimmens-css-control',
              },
              // Campo di ricerca
              createElement(TextControl, {
                label: siCssDataUi.labels.search || 'Search Classes',
                value: searchTerm,
                onChange: setSearchTerm,
                placeholder: siCssDataUi.labels.ttos || 'Type to search...',
              }),
                ...
                ? createElement(
                    ...
                    filtered.length > 0
                      ...
                      : createElement(
                          'p',
                          {},
                          siCssDataUi.labels.nofound || 'No classes found'
                        )
                  )
                : null
            )
