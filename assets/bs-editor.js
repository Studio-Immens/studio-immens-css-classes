...
(function () {
  ...
  if (typeof siCssDataBs === 'undefined') {
    console.warn('siCssDataBs non definito, impossibile inizializzare il CSS Class Editor');
    return;
  }
  ...
  (function injectDynamicCSS() {
    if (typeof document === 'undefined' || typeof siCssDataBs === 'undefined') {
      return;
    }
    ...
    styleEl.textContent = siCssDataBs.classes
      .map(c => `${c}`)
      .join('\n');
  })();
  ...
  function AddCSSClassControl(settings) {
    ...
    settings.edit = function (props) {
      ...
      // Filtra le classi
      const filtered = siCssDataBs.classes.filter((cls) => 
        cls.toLowerCase().includes(searchTerm.toLowerCase())
      );

      // if ( filtered == siCssDataBs.classes.toLowerCase() ) {
      //   filtered = [];
      // }
        ...
        props.isSelected &&
          createElement(
            ...
            createElement(
              PanelBody,
              {
                title: siCssDataBs.labels.title || 'Bootstrap Classes',
                initialOpen: false,
                className: 'studioimmens-css-control',
              },
              // Campo di ricerca
              createElement(TextControl, {
                label: siCssDataBs.labels.search || 'Search Classes',
                value: searchTerm,
                onChange: setSearchTerm,
                placeholder: siCssDataBs.labels.ttos || 'Type to search...',
              }),
                ...
                ? createElement(
                    ...
                    filtered.length > 0
                      ...
                      : createElement(
                          'p',
                          {},
                          siCssDataBs.labels.nofound || 'No classes found'
                        )
                  )
                : null
            )
