...
(function () {
  ...
  if (typeof siCssDataSe === 'undefined') {
    console.warn('siCssDataSe non definito, impossibile inizializzare il CSS Class Editor');
    return;
  }
  ...
  (function injectDynamicCSS() {
    if (typeof document === 'undefined' || typeof siCssDataSe === 'undefined') {
      return;
    }
    ...
    styleEl.textContent = siCssDataSe.classes
      .map(c => `${c}`)
      .join('\n');
  })();
  ...
  function AddCSSClassControl(settings) {
    ...
    settings.edit = function (props) {
      ...
      // Filtra le classi
      const filtered = siCssDataSe.classes.filter((cls) => 
        cls.toLowerCase().includes(searchTerm.toLowerCase())
      );

      // if ( filtered == siCssDataSe.classes.toLowerCase() ) {
      //   filtered = [];
      // }
        ...
        props.isSelected &&
          createElement(
            ...
            createElement(
              PanelBody,
              {
                title: siCssDataSe.labels.title || 'Semantic Classes',
                initialOpen: false,
                className: 'studioimmens-css-control',
              },
              // Campo di ricerca
              createElement(TextControl, {
                label: siCssDataSe.labels.search || 'Search Classes',
                value: searchTerm,
                onChange: setSearchTerm,
                placeholder: siCssDataSe.labels.ttos || 'Type to search...',
              }),
                ...
                ? createElement(
                    ...
                    filtered.length > 0
                      ...
                      : createElement(
                          'p',
                          {},
                          siCssDataSe.labels.nofound || 'No classes found'
                        )
                  )
                : null
            )
